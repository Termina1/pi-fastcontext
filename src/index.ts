...
async function readConfigFile(file: string): Promise<FastContextConfigOverrides> {
  try {
    ...
    throw new Error(`Failed to read FastContext config ${file}: ${error?.message || error}`);
  }
}
...
async function resolveConfig(cwd: string, overrides: FastContextConfigOverrides = {}): Promise<FastContextConfig> {
...
const fcTools = [
  {
    ...
    function: {
      name: "GLOB",
      description: "Find repository files by a glob pattern. Pattern must be relative to repo root and must not start with '/'. Examples: '**/*.go', 'internal/**/*.go', '**/*auth*'.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "GREP",
      description: "Regex search over repository text. path is an optional relative file or directory; never use leading '/'.",
      parameters: {
        type: "object",
        properties: {
          pattern: { type: "string" },
          path: { type: "string", description: "Optional relative directory or file. Empty means repository root." },
        },
        required: ["pattern"],
      },
    },
  },
  {
    ...
    function: {
      name: "READ",
      description: "Read a repository file and return line-numbered contents. path must be relative to repo root and must not start with '/'.",
      parameters: {
        type: "object",
        properties: {
          path: { type: "string" },
...
type Citation = { path: string; start: number; end: number; line: string; exists: boolean; inBounds: boolean };
...
function normalizeRel(raw: string, root: string): string {
  let rel = stripAt(String(raw || "")).trim().replaceAll("\\", "/");
  rel = rel.replace(/^\/+/, "");
  rel = rel.replace(/^\.\//, "");
...
async function isDir(abs: string): Promise<boolean> {
  try { return (await fs.stat(abs)).isDirectory(); } catch { return false; }
}
...
async function readTool(root: string, args: Record<string, any>): Promise<string> {
  ...
  if (skipRel(rel) || !(await looksText(abs))) return `ERR: refused non-text/skipped file: ${rel}`;
...
async function grepTool(root: string, args: Record<string, any>): Promise<string> {
  ...
  for (const fileRel of files) {
    ...
    scanned++;
    let lines: string[];
    try { lines = (await fs.readFile(fileAbs, "utf8")).split(/\r?\n/); } catch { continue; }
  ...
  if (results.length === 0) return `No matches for ${pattern} in ${rel || "repo root"} after scanning ${scanned} text files.`;
  const more = results.length >= MAX_GREP_RESULTS ? `\n... [stopped after ${MAX_GREP_RESULTS} matches]` : "";
...
function normalizeToolCalls(msg: any): ToolCall[] {
  ...
  return calls.map((tc: any, idx: number) => {
    ...
    if (typeof fn.arguments === "string") {
      try { parsed = fn.arguments.trim() ? JSON.parse(fn.arguments) : {}; }
      catch { parsed = { _parse_error: fn.arguments }; }
    } else if (fn.arguments && typeof fn.arguments === "object") {
      parsed = fn.arguments;
    }
    return { id: tc.id || `call_${idx}`, name: String(fn.name || ""), arguments: parsed };
  });
}
...
async function chat(baseUrl: string, model: string, messages: Message[], tools: any[] | undefined, maxTokens: number, signal?: AbortSignal): Promise<any> {
  ...
  if (!res.ok) throw new Error(`FastContext HTTP ${res.status}: ${text.slice(0, 1000)}`);
  return JSON.parse(text);
}

async function runFastContext(options: RunOptions): Promise<{ text: string; details: any }> {
  const root = path.resolve(options.cwd);
  if (!(await isDir(root))) throw new Error(`Repository path not found or not a directory: ${root}`);

  const messages: Message[] = [
    { role: "system", content: systemPrompt },
    { role: "user", content: `Repository root basename: ${path.basename(root)}\nQuery: ${options.query}` },
  ];
  ...
  for (let turn = 1; turn <= options.maxTurns; turn++) {
    options.onUpdate?.({ content: [{ type: "text", text: `FastContext turn ${turn}/${options.maxTurns}...` }] });
    const response = await chat(options.baseUrl, options.model, messages, fcTools, options.maxTokens, options.signal);
  ...
  if (!final && options.forceFinal) {
    ...
    final = extracted.final;
    partialFinal = extracted.partial;
    if (!final) errors.push(`force_final: no <final_answer>; content=${String(msg.content || "").slice(0, 300)}`);
  }
  ...
  if (partialFinal) warnings.push("final_answer tag was not closed; parsed partial final answer");
  if (citations.length && valid.length !== citations.length) warnings.push(`${citations.length - valid.length}/${citations.length} citations failed validation`);
  ...
  return {
    text,
    details: { root, baseUrl: options.baseUrl, model: options.model, final, citations, validCitations: valid.length, toolCalls, failedTools, elapsedMs, promptTokens, completionTokens, warnings, transcript: options.includeTranscript ? transcript : undefined },
  };
}
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description: "Use local Microsoft FastContext via llama.cpp to do fast read-only codebase search. Returns compact file:line citations.",
    promptSnippet: "Fast read-only codebase search using local FastContext; returns file:line citations.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      query: Type.String({ description: "Natural-language code search/context query." }),
